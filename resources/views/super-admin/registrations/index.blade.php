              ...
              <td style="text-align:right;">
                @if($r->status == \App\Constants\RegistrationConstant::PENDING)
                  <div style="display:inline-flex;gap:5px;">
                    <form method="POST" action="{{ route('registrations.approve', hid($r)) }}" data-confirm="Setujui &amp; buat akun?">@csrf
                      <button class="btn btn-sm btn-primary"><i class="ti ti-check" style="font-size:13px;"></i> Setujui</button>
                    </form>
                    <form method="POST" action="{{ route('registrations.reject', hid($r)) }}" data-confirm="Tolak pendaftaran ini?">@csrf
